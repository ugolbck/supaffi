...
// The rule a link's code must satisfy lives in `src/lib/linkCode.ts`, shared
// with `validateLinkInput` (src/lib/affiliateLink.ts) so the two can never
// validate a code differently. Re-exported here so callers in this
// directory keep importing it from the signup flow's own validation module.
export { validateCode, type CodeValidationResult } from "@/lib/linkCode";
